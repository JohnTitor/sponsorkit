...
async function resolveAvatars(ships, fallbackAvatar) {
  return Promise.all(ships.map(async (ship) => {
    const data = await ohmyfetch.$fetch(ship.sponsor.avatarUrl, { responseType: "arrayBuffer" }).catch((e) => {
      if (typeof fallbackAvatar === "string")
        return ohmyfetch.$fetch(fallbackAvatar, { responseType: "arrayBuffer" });
      if (fallbackAvatar)
...
async function getPastSponsors(username) {
  ...
  let cursor = 1;
  do {
    const content = await ohmyfetch.$fetch(`https://github.com/sponsors/${username}/sponsors_partial?filter=inactive&page=${cursor++}`, { method: "GET" });
...
async function fetchGitHubSponsors(token, login, type, config) {
  ...
  do {
    ...
    const data = await ohmyfetch.$fetch(API$1, {
      method: "POST",
      body: { query },
      headers: {
...
async function fetchPatreonSponsors(token) {
  ...
  const userData = await ohmyfetch.$fetch(
  ...
  do {
    const sponsorshipData = await ohmyfetch.$fetch(sponsorshipApi, {
      method: "GET",
      headers: {
        "Authorization": `bearer ${token}`,
        "Content-Type": "application/json"
...
async function fetchOpenCollectiveSponsors(key, id, slug, githubHandle) {
  ...
  do {
    ...
    const data = await ohmyfetch.$fetch(API, {
      method: "POST",
      body: { query },
      headers: {
        "Api-Key": `${key}`,
